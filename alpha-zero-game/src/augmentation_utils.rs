...
pub fn rotate_90_bitvec(src: impl AsRef<[bool]>, dst: &mut BitVec, size: usize) {
    let mut dst_vec = vec![false; size * size];

    rotate_90(&src, &mut dst_vec, size);

    *dst = BitVec::from_iter(dst_vec.into_iter());
}

pub fn rotate_180_bitvec(src: impl AsRef<[bool]>, dst: &mut BitVec, size: usize) {
    let mut dst_vec = vec![false; size * size];

    rotate_180(&src, &mut dst_vec, size);

    *dst = BitVec::from_iter(dst_vec.into_iter());
}

pub fn rotate_270_bitvec(src: impl AsRef<[bool]>, dst: &mut BitVec, size: usize) {
    let mut dst_vec = vec![false; size * size];

    rotate_270(&src, &mut dst_vec, size);

    *dst = BitVec::from_iter(dst_vec.into_iter());
}

pub fn flip_horizontal_bitvec(src: impl AsRef<[bool]>, dst: &mut BitVec, size: usize) {
    let mut dst_vec = vec![false; size * size];

    flip_horizontal(&src, &mut dst_vec, size);

    *dst = BitVec::from_iter(dst_vec.into_iter());
}

pub fn flip_vertical_bitvec(src: impl AsRef<[bool]>, dst: &mut BitVec, size: usize) {
    let mut dst_vec = vec![false; size * size];

    flip_vertical(&src, &mut dst_vec, size);
